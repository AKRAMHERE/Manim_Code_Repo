...
class MatrixRotation(Scene):
    def construct(self):
        ...
        # Remove Example 1 title and content completely before moving to Example 2
    ...
    def show_example(self, initial_matrix, transposed_matrix, rotated_matrix):
        ...
        self.play(FadeOut(step_text))  # Remove previous step text
        self.wait(1)
        ...
        # Show Final Matrix
        final_matrix_mob = self.create_matrix(rotated_matrix, "Rotated Matrix (90° Clockwise)")
        self.play(Transform(matrix_mob, final_matrix_mob))
        self.wait(2)

    def clear_screen(self):
        """Completely fade out everything before starting a new example"""
    ...
    def get_transpose_swaps(self, n):
        """Dynamically calculates the swaps needed for transposition."""
        swaps = []
        for i in range(n):
